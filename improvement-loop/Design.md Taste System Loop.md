---
type: improvement-loop
status: active
source_package: "[[../Generated-Packages/Design.md Taste System/README|Design.md Taste System]]"
source_note: "[[../../Threads/Design.md as a Taste Layer for AI-Built Startups|Design.md as a Taste Layer for AI-Built Startups]]"
cadence: nightly
last_improved: 2026-05-24
next_focus: "Create a real VinClawLabs/Mission Control DESIGN.md sample and use it on one prototype."
tags: [improvement-loop, design-md, agent-workflow]
---

# Design.md Taste System Loop

## Purpose
Continuously turn the `Design.md` taste-layer idea into reusable VinClawLabs product-building infrastructure.

## Current artifacts
- [[../Generated-Packages/Design.md Taste System/README|Package]]
- [[../Prototypes/Design.md Taste System/README|Prototype]]
- [[../Infographics/Design.md Taste System Workflow|Infographic]]
- [[../Skills/design-md-taste-system/SKILL|Skill draft]]

## Nightly improvement queue
1. Draft a concrete `DESIGN.md` for VinClawLabs / Mission Control.
2. Generate one product screen from that recipe.
3. Add a before/after comparison against a generic AI screen.
4. Decide whether the skill draft should be promoted as a companion to the existing `design-md` skill.

## Latest useful change
- 2026-05-24: Created the first prototype, workflow infographic, and draft skill candidate.
- 2026-05-24: Added [[../Prototypes/Design.md Taste System/VinClawLabs Mission Control DESIGN.md Sample|VinClawLabs Mission Control DESIGN.md Sample]] so the next run can apply a real recipe to a prototype instead of only explaining the workflow.
